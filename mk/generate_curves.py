...
#
# THE SOFTWARE IS PROVIDED "AS IS" AND BRIAN SMITH AND THE AUTHOR DISCLAIMS
...
pub static PUBLIC_SCALAR_OPS: PublicScalarOps = PublicScalarOps {
    ...
    twin_mul_vartime: |g_scalar, p_scalar, p_xy, cpu| {
        twin_mul_vartime_inefficient(&PRIVATE_KEY_OPS, g_scalar, p_scalar, p_xy, cpu)
    },
...
prefixed_extern! {
    unsafe fn p%(bits)s_elem_mul_mont(
        r: *mut Limb,   // [COMMON_OPS.num_limbs]
        a: *const Limb, // [COMMON_OPS.num_limbs]
        b: *const Limb, // [COMMON_OPS.num_limbs]
    );

    unsafe fn p%(bits)s_point_add(
        r: *mut Limb,   // [3][COMMON_OPS.num_limbs]
        a: *const Limb, // [3][COMMON_OPS.num_limbs]
        b: *const Limb, // [3][COMMON_OPS.num_limbs]
    );
    unsafe fn p%(bits)s_point_mul(
        r: *mut Limb,          // [3][COMMON_OPS.num_limbs]
    ...
    unsafe fn p%(bits)s_scalar_mul_mont(
        r: *mut Limb,   // [COMMON_OPS.num_limbs]
        a: *const Limb, // [COMMON_OPS.num_limbs]
        b: *const Limb, // [COMMON_OPS.num_limbs]
    );
