...
fn sm2_sign(c: &mut Criterion) {
    ...
    group.bench_function("sm2_sign", |b| {
        let pkcs8 =
            signature::Sm2KeyPair::generate_pkcs8(&signature::SM2_SM3_FIXED_SIGNING, &rng)
                .unwrap();
        ...
        .unwrap();
        b.iter(|| key_pair.sign(&rng, black_box(b"benchmark message")).unwrap())
    });
...
criterion_group!(sm_benches, sm3_vs_sha256, sm4_gcm_seal, sm4_gcm_open, sm2_sign, sm2_verify);
