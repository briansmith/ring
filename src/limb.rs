...
pub fn limbs_less_than_limbs_constant_time(a: &[Limb], b: &[Limb]) -> LimbMask {
    ...
    unsafe { RING_limbs_lt_limbs(a.as_ptr(), b.as_ptr(), b.len()) }
}

#[inline]
pub fn limbs_are_zero_constant_time(limbs: &[Limb]) -> LimbMask {
    unsafe { RING_limbs_are_zero(limbs.as_ptr(), limbs.len()) }
}
...
pub fn limbs_reduce_once_constant_time(r: &mut [Limb], m: &[Limb]) {
    ...
    unsafe { RING_limbs_reduce_once(r.as_mut_ptr(), m.as_ptr(), m.len()) };
}

extern {
    fn RING_limbs_are_zero(a: *const Limb, num_limbs: c::size_t) -> LimbMask;
    fn RING_limbs_lt_limbs(a: *const Limb, b: *const Limb, num_limbs: c::size_t)
                           -> LimbMask;
    fn RING_limbs_reduce_once(r: *mut Limb, m: *const Limb,
                              num_limbs: c::size_t);
}
