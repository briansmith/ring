...
impl EncryptBlock for Key {
    fn encrypt_block(&self, mut block: Block) -> Block {
        ...
        let mut batch = Batch::from_bytes(core::slice::from_ref(&block));
        batch.encrypt(&sched, usize_from_u32(self.inner.rounds));
        batch.into_bytes(core::slice::from_mut(&mut block));
        block
    }
