...
fn seal_finish(
    ...
    if remainder_len > 0 {
        ...
        for (c, (p, k)) in padded_ct.iter_mut().zip(padded_pt.iter().zip(keystream.iter())) {
            *c = p ^ k;
        }
    ...
    auth.pre_finish(|pre_tag| {
        ...
        for (t, (g, k)) in tag_bytes.iter_mut().zip(pre_tag.iter().zip(tag_keystream.iter())) {
            *t = g ^ k;
        }
        Tag::from(tag_bytes)
    })
...
fn open_finish(
    ...
    if remainder.len() > 0 {
        ...
        let keystream = sm4_key.encrypt_block(*Iv::from(ctr).as_ref());
        let mut padded_pt = ZERO_BLOCK;
        for (p, (c, k)) in padded_pt.iter_mut().zip(padded_ct.iter().zip(keystream.iter())) {
            *p = c ^ k;
        }
