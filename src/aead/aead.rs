...
use {constant_time, err, init, polyfill};
...
pub fn open_in_place(key: &OpeningKey, nonce: &[u8], in_prefix_len: usize,
                     in_out: &mut [u8], ad: &[u8]) -> err::Result<usize> {
...
///
/// FThe input is `in_out[..(in_out.len() - out_suffix_capacity)]`; i.e. the
...
impl Key {
    ...
    fn init(&mut self, key_bytes: &[u8]) -> err::EmptyResult {
        init::init_once();
...
pub struct Algorithm {
    init: fn(ctx_buf: &mut [u8], key: &[u8]) -> err::EmptyResult,
    ...
    open: fn(ctx: &[u64; KEY_CTX_BUF_ELEMS], nonce: &[u8; NONCE_LEN],
             ...
             tag_out: &mut [u8; TAG_LEN], ad: &[u8]) -> err::EmptyResult,
...
fn check_per_nonce_max_bytes(in_out_len: usize) -> err::EmptyResult {
