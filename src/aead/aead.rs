...
pub fn open_in_place(key: &OpeningKey, nonce: &[u8], in_prefix_len: usize,
    ...
    let ciphertext_and_tag_len = try!(in_out.len()
        .checked_sub(in_prefix_len)
        .ok_or(error::Unspecified));
    let ciphertext_len = try!(ciphertext_and_tag_len.checked_sub(TAG_LEN)
        .ok_or(error::Unspecified));
    ...
    try!((key.key.algorithm.open)(&key.key.ctx_buf,
                                  nonce,
                                  in_out,
                                  in_prefix_len,
                                  &mut calculated_tag,
...
pub fn seal_in_place(key: &SealingKey, nonce: &[u8], in_out: &mut [u8],
    ...
    try!((key.key.algorithm.seal)(&key.key.ctx_buf,
                                  nonce,
                                  in_out,
                                  tag_out,
                                  ad));
...
pub struct Algorithm {
    ...
    seal: fn(ctx: &[u64; KEY_CTX_BUF_ELEMS],
             nonce: &[u8; NONCE_LEN],
             in_out: &mut [u8],
             tag_out: &mut [u8; TAG_LEN],
             ad: &[u8])
             -> Result<(), error::Unspecified>,
    open: fn(ctx: &[u64; KEY_CTX_BUF_ELEMS],
             nonce: &[u8; NONCE_LEN],
             in_out: &mut [u8],
             in_prefix_len: usize,
             tag_out: &mut [u8; TAG_LEN],
...
impl Algorithm {
    ...
    /// The maximum number of bytes that sealing operations may add to
    /// plaintexts. See also `MAX_OVERHEAD_LEN`.
    ///
...
mod tests {
    ...
    pub fn test_aead(aead_alg: &'static aead::Algorithm, file_path: &str) {
        ...
        test::from_file(file_path, |section, test_case| {
            ...
            let s_result = aead::seal_in_place(&s_key,
                                               &nonce[..],
                                               &mut s_in_out[..],
                                               max_overhead_len,
                                               &ad);
            ...
            for in_prefix_len in in_prefix_lengths.iter() {
                ...
                let o_result = aead::open_in_place(&o_key,
                                                   &nonce[..],
                                                   *in_prefix_len,
                                                   &mut o_in_out[..],
                                                   &ad);
    ...
    fn test_aead_nonce_sizes(aead_alg: &'static aead::Algorithm)
        ...
        let ciphertext_len = try!(aead::seal_in_place(&s_key,
                                                      &nonce[..nonce_len],
                                                      &mut to_open,
                                                      suffix_space,
                                                      &ad));
        ...
        // Nonce is the correct length.
        {
            ...
            assert!(aead::seal_in_place(&s_key,
                                        &nonce[..nonce_len],
                                        &mut in_out,
                                        suffix_space,
                                        &ad)
        ...
        }
        {
            ...
            assert!(aead::open_in_place(&o_key,
                                        &nonce[..nonce_len],
                                        prefix_len,
                                        &mut in_out,
                                        &ad)
