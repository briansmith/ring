...
//!
//! ```
//! use ring::{digest, err, hmac, rand};
//!
//! # fn main_with_result() -> err::EmptyResult {
...
use {constant_time, digest, err, rand};
...
impl SigningKey {
    ...
    pub fn generate(digest_alg: &'static digest::Algorithm,
                    rng: &rand::SecureRandom) -> err::Result<SigningKey> {
