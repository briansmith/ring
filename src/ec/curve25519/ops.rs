...
impl ExtPoint {
    ...
    pub fn from_encoded_point_vartime(encoded: &EncodedPoint) -> Result<Self, error::Unspecified> {
        ...
        Result::from(unsafe { RingCore_x25519_ge_frombytes_vartime(&mut point, encoded) })
            .map(|()| point)
    }
...
fn encode_point(x: Elem<T>, y: Elem<T>, z: Elem<T>) -> EncodedPoint {
    ...
    let sign_bit: u8 = unsafe {
        ...
        RingCore_x25519_fe_invert(&mut recip, &z);

        let mut x_over_z = Elem::zero();
        RingCore_x25519_fe_mul_ttt(&mut x_over_z, &x, &recip);

        let mut y_over_z = Elem::zero();
        RingCore_x25519_fe_mul_ttt(&mut y_over_z, &y, &recip);
        RingCore_x25519_fe_tobytes(&mut bytes, &y_over_z);

        RingCore_x25519_fe_isnegative(&x_over_z)
    };
...
extern "C" {
    fn RingCore_x25519_fe_invert(out: &mut Elem<T>, z: &Elem<T>);
    fn RingCore_x25519_fe_isnegative(elem: &Elem<T>) -> u8;
    fn RingCore_x25519_fe_mul_ttt(h: &mut Elem<T>, f: &Elem<T>, g: &Elem<T>);
    fn RingCore_x25519_fe_neg(f: &mut Elem<T>);
    fn RingCore_x25519_fe_tobytes(bytes: &mut EncodedPoint, elem: &Elem<T>);
    fn RingCore_x25519_ge_frombytes_vartime(h: &mut ExtPoint, s: &EncodedPoint) -> bssl::Result;
}
