...
use super::limbs;
use crate::{error::LenMismatchError, limb::Limb, polyfill::uninit::Uninit};
...
#[derive(Clone, Copy)]
#[repr(transparent)]
pub struct N0<'a>(&'a limbs::n0::Limbs);

impl<'a> N0<'a> {
    pub(super) const LIMBS_USED: usize = limbs::n0::LIMBS_USED;

    #[allow(clippy::useless_conversion)]
    pub(super) fn write_into(
        out: Uninit<'_, limbs::n0::Limbs>,
        m: &[Limb],
    ) -> Result<(), LenMismatchError> {
        let (m_low_limbs, _) = m
            .split_first_chunk()
            .ok_or_else(|| LenMismatchError::new(m.len()))?;
        limbs::n0::write_n0_assuming_odd(out, m_low_limbs);
        Ok(())
    }

    pub(super) fn from_limbs_unchecked_less_safe(limbs: &'a limbs::n0::Limbs) -> Self {
        Self(limbs)
    }
}
