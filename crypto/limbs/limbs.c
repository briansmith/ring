...
Limb RingCore_LIMBS_less_than(const Limb a[], const Limb b[], size_t num_limbs);
Limb RingCore_LIMBS_less_than_limb(const Limb a[], Limb b, size_t num_limbs);
int RingCore_LIMBS_select_512_32(Limb r[], const Limb table[], size_t num_limbs,
                        crypto_word index);
crypto_word RingCore_LIMBS_window5_split_window(Limb lower_limb,
                                                Limb higher_limb,
...
Limb RingCore_LIMBS_equal_limb(const Limb a[], Limb b, size_t num_limbs) {
  ...
  Limb hi_zero = RingCore_LIMBS_are_zero(&a[1], num_limbs - 1);
...
Limb RingCore_LIMBS_less_than(const Limb a[], const Limb b[], size_t num_limbs) {
   ...
   * be consistent with |RingCore_LIMBS_limbs_reduce_once| and other code that makes such
...
Limb RingCore_LIMBS_less_than_limb(const Limb a[], Limb b, size_t num_limbs) {
  ...
  Limb hi = RingCore_LIMBS_are_zero(&a[1], num_limbs - 1);
...
void RingCore_LIMBS_reduce_once(Limb r[], const Limb m[], size_t num_limbs) {
  ...
  Limb lt = RingCore_LIMBS_less_than(r, m, num_limbs);
  Carry borrow =
...
void RingCore_LIMBS_add_mod(Limb r[], const Limb a[], const Limb b[], const Limb m[],
  ...
  Limb overflow2 = ~RingCore_LIMBS_less_than(r, m, num_limbs);
...
crypto_word RingCore_LIMBS_window5_split_window(Limb lower_limb, Limb higher_limb, size_t index_within_word) {
...
crypto_word RingCore_LIMBS_window5_unsplit_window(Limb limb, size_t index_within_word) {
