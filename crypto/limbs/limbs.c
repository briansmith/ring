...
Limb RING_limbs_lt_limbs(const Limb a[], const Limb b[], size_t num_limbs);
...
Limb RING_limbs_lt_limbs(const Limb a[], const Limb b[], size_t num_limbs) {
   ...
   * be consistent with |RING_limbs_reduce_once| and other code that makes such
...
void RING_limbs_reduce_once(Limb r[], const Limb m[], size_t num_limbs) {
  ...
  Limb lt = RING_limbs_lt_limbs(r, m, num_limbs);
  Carry borrow =
...
void RING_limbs_add_mod(Limb r[], const Limb a[], const Limb b[],
  ...
  Limb overflow2 = ~RING_limbs_lt_limbs(r, m, num_limbs);
