...
# void gcm_ghash_vpclmulqdq_avx2_1(uint8_t Xi[16], const u128 Htable[16],
#                                  const uint8_t aad[16], size_t aad_len_16);
#
# Using the key |Htable|, update the GHASH accumulator |Xi| with the data given
# by |in| and |len|.  |aad_len_16| must be exactly 16.
#
# This has the same signature `gcm_gmult_vpclmulqdq_avx2` but uses the
# implementation from `gcm_ghash_vpclmulqdq_avx2`, with the XOR of `aad` added.
$code .= _begin_func "gcm_ghash_vpclmulqdq_avx2_1", 1;
{
    # Function arguments (ghash signature)
    ...
    # XOR the AAD into the accumulator before byteswapping.
    vpxor           $T0, $GHASH_ACC, $GHASH_ACC
