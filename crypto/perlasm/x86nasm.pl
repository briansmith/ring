...
sub ::file_end
{   if (grep {/\b${nmdecor}RingCore_ia32cap_P\b/i} @out)
    {	my $comm=<<___;
${drdecor}segment	.bss
${drdecor}common	${nmdecor}RingCore_ia32cap_P 16
___
	# comment out RingCore_ia32cap_P declarations
	grep {s/(^extern\s+${nmdecor}RingCore_ia32cap_P)/\;$1/} @out;
	push (@out,$comm)
    }
